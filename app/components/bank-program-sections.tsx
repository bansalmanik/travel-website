...
const renderContentBlock = (block: ContentBlock, sectionId: string, index: number) => {
  ...
  return (
    <div key={`${sectionId}-block-bullets-${index}`} className="space-y-3">
      {block.title ? (
        <p className="text-xs font-semibold uppercase tracking-[0.3em] text-sky-200">{block.title}</p>
      ) : null}
      {renderBulletList(block.items, `${sectionId}-block-${index}`)}
    </div>
  );
};

export function BankProgramSections({ sections }: BankProgramSectionsProps) {
  if (!sections.length) {
    return null;
  }
      ...
      {sections.map((section) => {
        const gridColumnsClass = (() => {
          if (section.style === "key-values") {
            if (section.columns === 3) return "lg:grid-cols-3";
            if (section.columns === 1) return "lg:grid-cols-1";
          }
        ...
        return (
          ...
          >
            <div className="space-y-4">
              ...
              {section.intro ? <p className="text-sm text-slate-100/80">{section.intro}</p> : null}
                ...
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[340px] border-separate border-spacing-y-2 text-left text-sm text-slate-100/80">
                    {section.table.caption ? (
                      <caption className="pb-2 text-left text-xs uppercase tracking-[0.2em] text-sky-200">
                        {section.table.caption}
                      </caption>
                    ) : null}
                    <thead>
                      <tr>
                        {section.table.columns.map((column) => (
                          <th
                            ...
                            scope="col"
                            className="rounded-lg bg-white/10 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-sky-200"
                          >
                            {column}
                          </th>
                        ))}
              ...
              {section.style === "key-values" ? (
                <dl className={`grid gap-4 text-sm text-slate-100/80 ${gridColumnsClass}`}>
                  {section.items.map((item, index) => (
                    ...
                    >
                      <dt className="text-xs font-semibold uppercase tracking-[0.3em] text-sky-200">{item.label}</dt>
