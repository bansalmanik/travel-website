    ...
    <SectionWrapper title="Card snapshot">
      <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:gap-8">
        {card.media?.cardImage ? (
          <div className="mx-auto w-full max-w-[18rem] sm:mx-0">
            <figure className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 shadow-inner">
              <Image
              ...
              {card.media.cardImage.caption ? (
                <figcaption className="px-4 py-3 text-xs uppercase tracking-[0.2em] text-slate-500">
                  {card.media.cardImage.caption}
                </figcaption>
              ) : null}
            </figure>
...
function CardHighlights({ highlights }: { highlights?: string[] }) {
  ...
  return (
    <SectionWrapper title="Key highlights" description="Top perks that make this card stand out">
      <ul className="space-y-3 text-sm text-slate-700">
        {highlights.map((highlight) => (
          <li key={highlight} className="flex items-start gap-3 rounded-2xl bg-slate-50 p-4 shadow-inner">
            <span className="mt-1 h-2 w-2 flex-none rounded-full bg-blue-600" aria-hidden />
            <span>{highlight}</span>
          </li>
        ))}
...
function CardDetailSections({ card }: { card: Card }) {
  ...
  return (
    <div className="flex flex-col gap-6 sm:gap-8">
      ...
      <CardHighlights highlights={card.keyHighlights} />
